...
export const mq = {
    up: (bp: Breakpoint) => `(min-width: ${theme.breakpoints[bp]}px)`,
    down: (bp: Breakpoint) => `(max-width: ${theme.breakpoints[bp] - 1}px)`,
    between: (min: Breakpoint, max: Breakpoint) =>
        `(min-width: ${theme.breakpoints[min]}px) and (max-width: ${theme.breakpoints[max] - 1}px)`,
};
