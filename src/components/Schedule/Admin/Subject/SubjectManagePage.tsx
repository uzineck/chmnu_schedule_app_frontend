...
import SubjectRow from "./SubjectRow.tsx";
...
const SubjectManagePage: React.FC = () => {
    ...
    const fetchPage = useCallback(
        async (offset: number, search: string, generation: number) => {
            try {
                const response = await getListOfSubjects(
                ...
                if (generation !== requestGenRef.current) return;
                const text = error instanceof ApiCallError ? error.message : "Не вдалось завантажити дисципліни";
    ...
    const handleCreated = (subject: Subject) => {
        setItems((prev) => [subject, ...prev]);
        setTotal((prev) => (prev === null ? null : prev + 1));
    };
    const handleUpdated = (subject: Subject) => {
        setItems((prev) => prev.map((s) => (s.uuid === subject.uuid ? subject : s)));
    };
    const handleDeleted = (uuid: string) => {
        setItems((prev) => prev.filter((s) => s.uuid !== uuid));
            ...
            if (hasQuery) {
                return (
                    <EmptyState>
                        ...
                        Перевірте написання або створіть нову дисципліну.
                    </EmptyState>
                );
            }
            return (
                <EmptyState>
                    Жодної дисципліни ще не створено.<br />
