...
const RoomManagePage: React.FC = () => {
    ...
    const fetchPage = useCallback(
        async (offset: number, search: string, generation: number) => {
            try {
                const response = await getListOfRooms(
                ...
                if (generation !== requestGenRef.current) return;
                const text = error instanceof ApiCallError ? error.message : "Не вдалось завантажити аудиторії";
    ...
    const handleCreated = (room: Room) => {
        setItems((prev) => [room, ...prev]);
        setTotal((prev) => (prev === null ? null : prev + 1));
    };
    const handleUpdated = (room: Room) => {
        setItems((prev) => prev.map((r) => (r.uuid === room.uuid ? room : r)));
    };
            ...
            if (hasQuery) {
                return (
                    <EmptyState>
                        ...
                        Перевірте написання або створіть нову аудиторію.
                    </EmptyState>
                );
            }
            return (
                <EmptyState>
                    Жодної аудиторії ще не створено.<br />
