...
const GroupManagePage: React.FC = () => {
    const [items, setItems] = useState<GroupWithHeadman[]>([]);
    ...
    const fetchPage = useCallback(
        async (offset: number, search: string, generation: number) => {
            try {
                ...
                if (generation !== requestGenRef.current) return;
                const text = error instanceof ApiCallError ? error.message : "Не вдалось завантажити групи";
    ...
    const handleCreated = (group: GroupWithHeadman) => {
        setItems((prev) => [group, ...prev]);
        setTotal((prev) => (prev === null ? null : prev + 1));
    };
    const handleUpdated = (group: GroupWithHeadman) => {
        setItems((prev) => prev.map((g) => (g.uuid === group.uuid ? group : g)));
    };
    const handleDeleted = (uuid: string) => {
        setItems((prev) => prev.filter((g) => g.uuid !== uuid));
            ...
            if (hasQuery) {
                return (
                    <EmptyState>
                        ...
                        Перевірте написання або створіть нову групу.
                    </EmptyState>
                );
            }
            return (
                <EmptyState>
                    Жодної групи ще не створено.<br />
                    Натисніть «Створити», щоб додати першу.
                </EmptyState>
            );
        }
        ...
        <Page>
            ...
            <Controls>
                <SearchWrap>
                    ...
                    <SearchInput
                        type="search"
                        placeholder="Пошук за номером групи..."
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                        aria-label="Пошук груп"
                    />
                </SearchWrap>
